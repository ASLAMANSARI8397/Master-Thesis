...
cap = cv2.VideoCapture("/Users/jarvis/Desktop/project/Input/5.mp4")
...
csv_file = "robot_lateral_tracking_with_ID.csv"
tracking_data = []
...
while True:
    ...
    for object_id, (pcx, pcy) in objects.items():
        if (pcx, pcy) not in bbox_map:
            continue  # not in field of interest
        ...
        cv2.putText(frame, f"ID:{object_id} {quadrant}",
                    (px1, py1 - 12),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2)
        cv2.putText(frame, f"Lat:{lat_cm}cm",
                    (px1, py1 + 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2)
...
if not df.empty:
    fig = px.scatter(
        ...
        color=df["Quadrant"],
        hover_data=["ID"],
        title="Lateral Distance Over Time (Robot Centered)"
    )
    fig.update_layout(
        xaxis_title="Time (Seconds)",
        yaxis_title="Lateral Distance from Robot (cm)",
        hovermode="closest"
    )
    fig.show()
else:
    print("❗ No valid data to plot. CSV empty.")