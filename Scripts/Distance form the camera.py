...
df.to_csv("tracking_data_below_400cm.csv", index=False)
print("✅ Tracking data saved to tracking_data_below_400cm.csv!")

# ✅ Plot
if not df.empty:
    df['Danger'] = df['Distance to Camera (cm)'] < DISTANCE_THRESHOLD
    fig = px.scatter(
        df,
        x="Timestamp (s)",
        y="Distance to Camera (cm)",
        color=df['Danger'].map({True: 'Danger (Red)', False: 'Safe (Blue)'}),
        color_discrete_map={'Danger (Red)': 'red', 'Safe (Blue)': 'blue'},
